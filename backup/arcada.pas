...
for q:=1 to 300 do
begin
      if (Zmeyka[q].x=Zmeyki[i,j].x) and (Zmeyka[q].y=Zmeyki[i,j].y) then
begin
      //---Поражение---//
...
For j:=1 to 3 do
Begin
      if j=1 then Buf.Canvas.Draw(Zmeyki[i,j].x*16,Zmeyki[i,j].y*16,IMG[3]) else  Buf.Canvas.Draw(Zmeyki[i,j].x*16,Zmeyki[i,j].y*16,IMG[4]);
      end;
   end;
end;
...
for i:=1 to 1 do
begin
       if (u=False) and (Zmeyki[i,1].x=-2) then
begin
       u:=True;
